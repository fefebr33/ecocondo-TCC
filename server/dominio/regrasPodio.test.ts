...
describe("classificarPodio", () => {
  ...
  it("divide a posição e mantém os dois empatados no 3º lugar elegíveis ao desconto", () => {
    const resultado = classificarPodio([linha(1, 10, 1), linha(2, 8, 1), linha(3, 5, 2000), linha(4, 5, 2000), linha(5, 4, 9000)]);
    expect(resultado.map((item) => [item.moradorId, item.posicao, item.elegivelDesconto])).toEqual([
