...
import { Mail, Pencil, Plus, QrCode, Search, UserRoundCheck, UserRoundX, UsersRound } from "lucide-react";
...
export default function Residents() {
  ...
  const visibleResidents = useMemo(() => (residents ?? []).filter((resident) => `${resident.nome} ${resident.bloco} ${resident.apartamento}`.toLowerCase().includes(query.toLowerCase())), [residents, query]);
  const [qrTarget, setQrTarget] = useState<{ id: number; nome: string } | null>(null);
  const qrCode = trpc.moradores.codigoQr.useQuery({ id: qrTarget?.id ?? 0 }, { enabled: qrTarget !== null });
      ...
      <DialogContent className="sm:max-w-sm">
        ...
        <div className="grid place-items-center gap-3 py-2">
          {qrCode.isLoading ? <p className="text-sm text-muted-foreground">Gerando código...</p> : qrCode.data ? (
            <>
              <img src={qrCode.data.qrDataUrl} alt={`QR code do apartamento de ${qrTarget?.nome}`} className="h-48 w-48 rounded-xl border border-[#e2ebe5]" />
              <p className="rounded-lg bg-[#f6faf7] px-3 py-1.5 text-sm font-mono font-semibold tracking-widest text-[#0f7350]">{qrCode.data.code}</p>
            </>
          ) : <p className="text-sm text-destructive">Não foi possível gerar o código.</p>}
        </div>
      </DialogContent>
    </Dialog>
  </div>;
