...
export default function Sustainability() {
  ...
  const compliance = trpc.conformidade.visaoGeral.useQuery(undefined, { enabled: isAdmin });
  const comparison = trpc.comparacao.porBloco.useQuery(undefined, { enabled: isAdmin });
  const timeline = trpc.comparacao.linhaDoTempo.useQuery(undefined, { enabled: isAdmin });
  ...
  const refresh = () => {
    void utils.metas.listar.invalidate();
    void utils.ocorrencias.listar.invalidate();
    void utils.conformidade.visaoGeral.invalidate();
    void utils.comparacao.porBloco.invalidate();
    void utils.comparacao.linhaDoTempo.invalidate();
  };
  const createGoal = trpc.metas.criar.useMutation({ onSuccess: () => { toast.success("Meta criada para o bloco."); refresh(); setGoal((current) => ({ ...current, title: "Meta mensal de recicláveis" })); }, onError: (error) => toast.error(error.message) });
  ...
  const onImageChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    reduzirFoto(file).then((imageDataUrl) => setIncident((current) => ({ ...current, imageDataUrl })), (issue) => toast.error(issue instanceof Error ? issue.message : "Não foi possível usar a imagem."));
  };
  ...
  return <div>
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
      <article className="rounded-[24px] border border-[#dce8e0] bg-white p-5 shadow-[0_16px_34px_-28px_rgba(4,66,42,.35)] sm:p-6">
        ...
        <div className="mt-5 grid gap-3">{goals.data?.length ? goals.data.map((item) => <div key={item.id} className="rounded-2xl border border-[#e0ebe4] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-semibold">{item.titulo}</p><p className="mt-1 text-sm text-muted-foreground">Bloco {item.bloco} · {formatDate(item.dataInicio)} a {formatDate(item.dataFim)}</p></div><Badge className="border-0 bg-[#e8f4ed] text-[#0a7048] hover:bg-[#e8f4ed]">{formatarNumero(item.collectedKg)} / {formatarNumero(item.metaKg)} kg</Badge></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e8efeb]"><div className="h-full rounded-full bg-[#0f7350] transition-all" style={{ width: `${item.progress}%` }} /></div><div className="mt-2 flex items-center justify-between text-xs text-muted-foreground"><span>{item.progress}% da meta</span>{isAdmin && <button onClick={() => toggleGoal.mutate({ id: item.id, isActive: !item.ativo })} className="font-semibold text-[#0f7350] hover:underline">{item.ativo ? "Pausar" : "Reativar"}</button>}</div></div>) : <p className="rounded-2xl bg-[#f6faf7] p-5 text-sm leading-6 text-muted-foreground">Ainda não há metas criadas. As metas ajudam a orientar a participação por bloco e são atualizadas com coletas concluídas.</p>}</div>
      </article>
    ...
    <section className="mt-6 rounded-[24px] border border-[#dce8e0] bg-[#103f2e] p-5 text-white shadow-[0_16px_34px_-28px_rgba(4,66,42,.45)] sm:p-6">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <span className="rounded-2xl bg-white/10 p-4"><TreePine className="h-4 w-4 text-[#91d7ae]" /><b className="mt-2 block text-2xl">{formatarNumero(summary.data?.equivalencias.arvoresPoupadas ?? 0, 1)}</b><span className="text-sm text-[#c6e4d1]">árvore(s) poupada(s)</span></span>
        ...
        <div className="mt-5 grid gap-3">
          {myGoals.data?.length ? myGoals.data.filter((item) => item.ativo).map((item) => (
            <div key={item.id} className="rounded-2xl border border-[#e0ebe4] p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">{item.progresso}% da meta{item.atingida ? " · Meta atingida! 🎉" : ""} · equivale a {formatarNumero(item.equivalencias.arvoresPoupadas, 1)} árvore(s) poupada(s)</p>
            </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {certificates.data?.length ? certificates.data.map((item) => (
          <a key={item.id} href={item.urlArquivo} target="_blank" rel="noreferrer" className="flex items-center justify-between gap-3 rounded-2xl border border-[#e5eee8] bg-[#fbfdfc] px-4 py-3 text-sm hover:border-[#0f7350]">
            <span><b className="block font-semibold">Bloco {item.bloco} · {item.trimestre}</b><span className="text-xs text-muted-foreground">{formatarNumero(item.pesoKg, 1)} kg reciclados</span></span>
            <Badge className="border-0 bg-[#fff3df] text-[#7a4d0a] hover:bg-[#fff3df]">Baixar PDF</Badge>
          </a>
